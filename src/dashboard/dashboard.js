...
const loadPlaylist = async (endpoint, elementId) => {
  ...
  for (let { name, description, images, id } of items) {
    ...
    playlistItem.className =
      "rounded p-4 hover:cursor-pointer hover:bg-gray bg-light-black";
    playlistItem.id = id;
    ...
    playlistItem.innerHTML = `<img class="rounded mb-2 object-contain shadow " src="${imageUrl}" alt="${name}" />
...
const fillContentForDashboard = () => {
  ...
  coverContent.innerHTML = `<h1 class="text-6xl font-extrabold pt-6">Hello ${displayName}...</h1>`;
...
const fillContentForPlaylist = async (playlistId) => {
  ...
  <img src="${images[0].url}" alt="" class="object-contain h-48 w-48" />
          <section class=" gap-3 pt-18">
            <p>PLAYLIST</p>
            <h2 id="playlist-name" class="font-extrabold text-8xl">${name}</h2>
...
const playTrack = (
  event,
  { image, name, artistNames, duration, previewUrl, id }
) => {
...
const loadPlaylistTracks = ({ tracks }) => {
  ...
  for (let trackItem of tracks.items.filter((item) => item.track.preview_url)) {
         ...
         <span class ="track-no">${trackNo++}</span>
           ...
           <section class="grid grid-cols-[auto_1fr] place-items-center gap-2">
             ...
             <article class="flex flex-col justify-center gap-2">
                <h3 class="song-title text-base text-primary line-clamp-1">${name}</h3>
    ...
    playButton.addEventListener("click", (event) =>
      playTrack(event, { image, name, artistNames, duration, previewUrl, id })
    );
  ...
  setItemsInLocalStorge(LOADED_TRACKS, loadedTracks);
};
